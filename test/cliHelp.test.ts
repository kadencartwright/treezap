...
test("running without a subcommand shows help", () => {
  ...
  assert.match(help, /treezap 0\.1\.5/);
  assert.match(help, /Usage:\n {2}treezap <command> \[options\]/);
...
test("help shows the raw command surface", () => {
  ...
  assert.match(
    help,
    /candidates <root> \[--min-age duration\] \[--count\]\s+List aged worktrees with deletion safety facts\./,
  );
  assert.match(
    help,
    /rm <path> \[--min-age duration\]\s+Delete one eligible worktree\./,
  );
  assert.match(
    help,
    /rm-old <root> \[--min-age duration\]\s+Delete eligible linked worktrees\./,
  );
